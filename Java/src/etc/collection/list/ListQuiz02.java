...
public class ListQuiz02 {
	...
	public static void main(String[] args) {
        ...
        5. 리스트 내부 객체 중 이름이 "홍길동" 이 있다면 그 객체를 삭제해 주세요. (일반 for문)
		 */


		List<User> userList = new ArrayList<>();

		User u1 = new User("홍길동", 32);
		User u2 = new User("김철수", 43);
		User u3 = new User("최미란", 46);

		Collections.addAll(userList, u1, u2, u3);

		//		userList.add(new User("홍길동", 20));
		//		userList.add(new User("김철수", 43));
		//		userList.add(new User("최미란", 46));
...
//			}
		...
		/*
		 - forEach(향상 for문)을 사용해 반복문을 순회할 때 리스트의 크기를 마음대로 줄이거나 늘릴 수 없다.
		 - for문은 반복문을 시작할 때 구조적으로 컬렉션의 크기 및 사용가능한 요소들의 개수를 항상 파악하고 있기 때문에
		   값의 변경이 일어나면 예외를 발생시키도록 설계돼있다.
		 */
