...
public class InputSrieamEx {

	public static void main(String[] args) {


		/*
		 1. 파일을 읽어들이는데 사용하는 클래스느  FileInputstream이다.
		 2. 생성자의 매개값으로 읽어들일 파일의 전체 경로를 적는다.
		 3. InputStream 객체는 생성자에 throws가 있기 때문에 예외처리를 진행해야 핟나.
		 */
		...
		try {
			fis = new FileInputStream("C:\\Work\\file\\hoy.txt");
			...
			*/
			...
			try {
				//스트림을 더 이상 사용하지 않는 경우 시스템 자원을 반납하는 코드를 반드시 작성해야 한다.
				fis.close();
			}
			catch (IOException e) {
				e.printStackTrace();			
