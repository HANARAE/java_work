...
/*
 ...
 3. 매개 값을 몇 개 받아서 사용할지는 메서드를 선언할 때 선언부에서 ()안에 미리 개수와 타입을 지정한다.
 
 4. 매개변수의 이름을 지을때는 호출자가 선언부로 돌아와서 매개값을 확인하지 않게 
    어떠한 값을 담는 변수인지 그 뜻을 알기 쉽게 지어주면 서로에게 좋다.
    
 5. 매개변수를 하나도 선언하지 않을수도 있다. 이 때는 메서드의 선언부에 () 내부를 비워둔다.
...
public class MethodExample2 {
	...
	static String selectRandomFood() {
		double rn = Math.random(); //범위: 0.0<rn<1.0
		if(rn > 0.66) {
			return "치킨";
		} else if(rn > 0.33) {
			return "족발";
...
//		}
	...
	//콤마로 나열되어 들어오는 여러개의 값을 배열로 묶어서 내부로 전달함.
	static int calcNumTotal(int... nums) {
		int total = 0;
		for(int n : nums) {
			total += n;
		}
	...
	public static void main(String[] args) {
		...
		//가변 파라미터에 값을 보낼때는 콤마로 쭉 나열하거나 배열로 포장해서 보낸다.
		sum = calcNumTotal(1,2,3,4,5);
		System.out.println("누적합: " + sum);
